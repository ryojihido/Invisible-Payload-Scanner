...
1. `dist/` にリリースzipを作成する（例: `InvisiblePayloadScanner-v0.4.0.zip`）。
2. SHA-256を算出する:

   ```powershell
   Get-FileHash .\dist\InvisiblePayloadScanner-v0.4.0.zip -Algorithm SHA256
   ```

3. 算出したSHA-256を **GitHub Releaseノートに必ず記載する**。
